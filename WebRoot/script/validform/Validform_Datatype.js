/*
	...
	idcard：对身份证号码进行严格验证
	"integer":"请填写大于零的整数",
    "ppt_suffix":"请上传PPT格式文件",
    "photo_suffix":"请上传图片!",
    "doc_suffix":"请上传doc或者pdf格式文件",
    "empty": ""
*/
...
(function () {
    if ($.Datatype) {
        $.extend($.Tipmsg.w, {
            ...
            "photo_suffix":"请上传图片!",
            "doc_suffix":"请上传doc或者pdf格式文件",
            "doc_only_suffix":"请上传doc或docx格式文件",
            "excel_suffix":"请上传excel格式文件",
            "file_size":"上传文件过大!",
            "empty": "",
            "email": "请填写邮箱！",
            "intl_tel": "请填写正确手机号！",
        ...
        $.extend($.Datatype, {
			...
			*/
            ...
            "c":/^(\d{3,4}-)?\d{7,8}$/,//传真或电话  
            "photo_suffix":function(gets,obj,curform,regxp){
				 var rgx=/(jpg|gif|JPG|GIF|JPEG|PNG|BMP|jpeg|png|bmp)/i;  
				 var sFilePath=obj.val();
			...
			"ppt_suffix":function(gets,obj,curform,regxp){
				 var rgx=/(ppt|PPT|pptx|PPTX)/i;  
				 var sFilePath=obj.val();
				 var ext=sFilePath.substring(sFilePath.lastIndexOf(".")+1);         
				 if(rgx.test(ext)) 
					 return true;
			...
			"doc_suffix":function(gets,obj,curform,regxp){
				 var rgx=/(doc|docx|DOC|DOCX|PDF|pdf)/i;  
				 var sFilePath=obj.val();
				 var ext=sFilePath.substring(sFilePath.lastIndexOf(".")+1);         
				 if(rgx.test(ext)) 
					 return true;
			...
			"doc_only_suffix":function(gets,obj,curform,regxp){
				 var rgx=/(doc|docx)/i;  
				 var sFilePath=obj.val();
				 var ext=sFilePath.substring(sFilePath.lastIndexOf(".")+1);         
				 if(rgx.test(ext)) 
			...
			"excel_suffix":function(gets,obj,curform,regxp){
				 var rgx=/(xls|xlsx|XLS|XLSX)/i;  
				 var sFilePath=obj.val();
				 var ext=sFilePath.substring(sFilePath.lastIndexOf(".")+1);         
				 if(rgx.test(ext)) 
					 return true;
			...
			"file_size":function(gets,obj,curform,regxp){
				//根据file-size获取最大大小,如果没有默认5M
				var fileSize=document.getElementsByName(obj.attr("name"))[0].getAttribute("file-size");
				if(!fileSize){
					fileSize=5;
				}
				var msg=isFileLegal(document.getElementsByName(obj.attr("name"))[0],null,fileSize);
				if(msg!="")
					return false;
				else
					return true;
			...
			"intl_tel":function(gets,obj,curform,regxp){
				try{
					if($(obj).attr("ignore")){
						return true;
					}
...
/**
 *上传文件是否合法 用法如：var aa=isFileLegal(document.getElementById("photo"),".jpg,.png",0.1); alert(aa)
 *默认最大为5M
 *
 */
function isFileLegal(target,fileTypes,filemaxsizePara) {
	...
	var filemaxsize = 1024 * 5;//默认5M 
	if(filemaxsizePara){
		filemaxsize = 1024 *filemaxsizePara;//2M 
	}
	...
	if (size > filemaxsize) {
		target.value = "";
		return "文件大小不能大于" + filemaxsize/ 1024 + "M！";
	}
	if (size <= 0) {
		target.value = "";
		return "文件大小不能为0M！";
...
//身份证vilidateForm验证
var validate_idCard=function(gets,obj,curform,datatype){
   //该方法由佚名网友提供;
   var Wi = [ 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2, 1 ];// 加权因子;
   var ValideCode = [ 1, 0, 10, 9, 8, 7, 6, 5, 4, 3, 2 ];// 身份证验证位值，10代表X;
   if (gets.length == 15) {   
...
//datatype  ppt 格式验证
var datatype_ppt_suffix=function(gets,obj,curform,regxp){
					 var rgx=/(ppt|PPT|pptx|PPTX)/i;  
					 var sFilePath=obj.val();
					 var ext=sFilePath.substring(sFilePath.lastIndexOf(".")+1);         
					 if(rgx.test(ext)) 
...
//datatype  doc 格式验证
var datatype_doc_suffix=function(gets,obj,curform,regxp){
					 var rgx=/(doc|docx|DOC|DOCX|PDF|pdf)/i;  
					 var sFilePath=obj.val();
					 var ext=sFilePath.substring(sFilePath.lastIndexOf(".")+1);         
					 if(rgx.test(ext)) 
						 ...
						 return "请上传doc或者pdf格式文件";
					
};
var Validform_for_tipMsg=function(msg,o,cssctl){
	var msg=msg.replace(/\s/g,"");
	 //提示字,如果是radio 或者checkbox 用了插件的话,得加在提示元素上Validform-for=name 属性
		if($("[Validform-for='"+$(o.obj).attr("name")+"']").attr("Validform-for")){
			var objtip= $("[Validform-for='"+$(o.obj).attr("name")+"']");
