...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		function getInterval()  {
			...
			var np = numPages();//总页数
			var upper_limit = np-opts.num_display_entries;//要显示几个分页数
		...
		function drawLinks() {
			...
			var appendItem = function(page_id, appendopts){
				...
				if(page_id == current_page-1){
					var lnk = jQuery("<li class='current'><a>"+(appendopts.text)+"</a></li>");
				}else{
					var lnk =jQuery("<li></lli>");
					var a_lnk = jQuery("<a>"+(appendopts.text)+"</a>")
			...
			if(opts.prev_text && (current_page > 1 || opts.prev_show_always)){
				appendItem(current_page-2,{text:opts.prev_text, classes:"prev"});
			}
			// 产生起始点
