<#macro echarts url docId="echarts1" tagBody="false" title="" 
	...
	xAxisName="" yAxisName="" number=1 dataZoomDisabled="true" >
<#-- 
...
<script type="text/javascript">
	var ecConfig;
	var myChart${docId};//图表实体变量 这个不能代码化到插件里面，不然没法绑定事件
	importJSCSS("/script/echarts3.0/echarts.js");
	//引入中国地图 如果需要某个省市地区的 需要单独引入
	importJSCSS("/script/echarts3.0/china.js");
	//引入对应主题js
	importJSCSS("/script/echarts3.0/theme/${topic}.js");
	...
	var echarts${docId!}Plugin={
		...
		initSetting:function(){
			var setting={
					...
					//不能用echarts.init(document.getElementById("${docId}"),"${topic}") 替换，否则无法绑定事件
					...
					grid:{x:"4%",x2: "5%"},
					dataZoomDisabled:${dataZoomDisabled}
				}
			return setting;
		},
		//2.0初始化注册用户统计表
		initChart:function (condition) {
			...
			else if(this.initSetting().chartType=='horizbar')
				this.initChartHerBar(condition);
		},
		//2.2刷新数据 
		...
		finalInit:function (option){
			...
			this.initSetting().myChart.on("click", ${clickFunc});
			</#if>
			...
			$.getJSON(this.initSetting().url,conditions, function(data) {
				if(data instanceof Array){
					for (var i = 0; i < data.length; i++) {
						if(chartType=='line'||chartType=='bar'||chartType=='horizbar'){
	...
	//------------------------------------------------------不同图表不同初始化方法区----------------------------------------------------	
		...
		initChartHerBar:function (condition){
			//1.获取数据
			var conditions = eval('(' + condition + ')');
			var valuesMap = this.getData(conditions);
			var keys = valuesMap.keys.get(0);
			 ...
			 /*    dataZoom: [{
				...
				xAxis : [ {
					type : 'value',
					  splitLine: {
		                    show: true,
		                    lineStyle: {color: "rgba(255,255,255,.5)"}
		                },
					axisLine:{lineStyle: {color: "rgba(255,255,255,.5)"}}
				} ],
				yAxis : [ {
