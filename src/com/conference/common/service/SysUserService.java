...
public class SysUserService extends BaseService<SysUser> {
	...
	public SysUserService(){super(SysUser.dao);}//调用父级构造函数，将dao传入service
...
//-----------------------------------------主要方法区---------------------------------------------------	
	 ...
	 */
	public Map<String, String> dealReg(String realname,String nickname,String email,String mobile,String password,BaseController c,Boolean isAutoLogin,Boolean checkMobileVc,boolean isMd5Pwd,String regType){
	 ...
	 */
	public Map<String, String> dealReg(SysUser account,BaseController c,Boolean isAutoLogin,Boolean checkVc,boolean isMd5Pwd,String regType){
		Map<String,String> returnMsg=new HashMap<String, String>();
		if(account==null){
			returnMsg.put(Constant.STATUS, (String) c.getAttr(Constant.STATUS));
			returnMsg.put(Constant.MSG, (String) c.getAttr(Constant.MSG));
	 ...
	 */
	public boolean findPwd(BaseController c,String validateCode,String imgValidataCode,String mobile,String email,String password,String regType){
//		1.判断手机号或者邮箱是否都为null
	...
	public boolean checkImageCodeAndVc(BaseController c,String checkTo,String checkVc,Boolean checkMobileVc){
		...
		if(StrKit.notBlank(validataCode)){
			if(!HttpRequestDeviceUtils.isMobileDevice(c.getRequest())){
				if(!validataCode.toLowerCase().equals(c.getSessionAttr(Constant.SESSION_VALIDATECODE).toString().toLowerCase())){
					c.setCommonMsg("验证码错误！");
					c.renderForReg();
					return false;
				}
			}
...
//		2.检测账户密码是否正确,账户是否存在
		...
		if(account==null){
			...
			if(temp!=null){
				...
				c.setCommonMsg(login_id+"用户没有注册！");
			}
			returnMsg.put(Constant.STATUS, (String) c.getAttr(Constant.STATUS));
			returnMsg.put(Constant.MSG, (String) c.getAttr(Constant.MSG));
			return null;
