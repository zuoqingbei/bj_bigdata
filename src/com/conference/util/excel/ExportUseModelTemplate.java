 ...
 * @todo   新闻使用方式导出模板
 */
public class ExportUseModelTemplate extends AbstractExcelExportTemplate<RdcProductAgentDetail>{
	...
	public ExportUseModelTemplate(List<Record> list,List<Record> useTypes,String title) {
		this.list = list;
	...
	protected void buildTitle(int sheetIndex) {
		...
		columnWidths[1]=3000;
		columnWidths[2]=4500;
		for(int x=0;x<useTypes.size()*3;x++){
	...
	protected void buildBody(int sheetIndex) {
		...
		Sheet sheet = getSheet(sheetIndex);
		//设置表头
		String[] tit1=new String[pre+useTypes.size()*3];
		tit1[0]="序号";
		tit1[1]="时间";
		tit1[2]="部门(发稿人)";
		for(int x=0;x<useTypes.size()*3;x++){
		...
		String[] tit2=new String[pre+useTypes.size()*3];
		tit2[0]="序号";
		tit2[1]="时间";
		tit2[2]="部门(发稿人)";
		for(int x=0;x<useTypes.size()*3;x++){
			tit2[pre+x]=useTypes.get(x/3).getStr("use_type_id");
		}
		String[] tit3=new String[pre+useTypes.size()*3];
		tit3[0]="序号";
		tit3[1]="时间";
		tit3[2]="部门(发稿人)";
		int index=3;
		...
		int size=3+1+useTypes.size()+useTypes.size()*3;
		int[][] mergeCells = new int[size][];
		mergeCells[0]=new int[]{0,2,0,0};
		mergeCells[1]=new int[]{0,2,1,1};
		mergeCells[2]=new int[]{0,2,2,2};
		mergeCells[3]=new int[]{0,0,3,useTypes.size()*3+3-1};
		//使用方式
		for(int x=0;x<useTypes.size();x++){
			mergeCells[4+x]=new int[]{1,1,3*(x+1),3*(x+1)+3-1};
		};
		//评价等级
		for(int x=0;x<useTypes.size()*3;x++){
			mergeCells[4+useTypes.size()+x]=new int[]{2,2,3+x,3+x};
		}
		//合并表头单元格 行行列列
